...
byte         DW1000RangingClass::_currentShortAddress[4];
...
void DW1000RangingClass::startAsAnchor(char address[], const byte mode[]) {
	...
	_currentShortAddress[0] = 254;
	_currentShortAddress[1] = 255;
	//_currentShortAddress[0] = random(0, 256);
	//_currentShortAddress[1] = random(0, 256);
	_currentShortAddress[2] = 1;
	_currentShortAddress[3] = 2;
...
void DW1000RangingClass::startAsTag(char address[], const byte mode[]) {
	...
	//_currentShortAddress[0] = random(0, 256);
	//_currentShortAddress[1] = random(0, 256);
	_currentShortAddress[0] = 190;
	_currentShortAddress[1] = 168;
...
void DW1000RangingClass::loop() {
	...
	if(_receivedAck) {
		...
		if(messageType == BLINK && _type == ANCHOR) {
			...
			_expectedMsgId = POLL;
			//****************
			//try out by checking if the message consists of an expected message ID call poll, we know that it is actually from anchor.
			//***************
		}else if(messageType == BLINK &&_type == ANCHOR && _expectedMsgId == POLL){
			byte address[8];
			byte shortAddress[4];
			_globalMac.decodeBlinkFrame(data, address, shortAddress);
			
			//we create a new device with anchor
			DW1000Device myAnchor(address, true);
			Serial.println("Anchor Blinking");
			...
			if(_type == ANCHOR) {
				...
				if(messageType == POLL) {
					...
					for(uint8_t i = 0; i <= numberDevices; i++) {
						...
						//we grab the mac address of each devices:
						byte shortAddress[4];
						memcpy(shortAddress, data+SHORT_MAC_LEN+2+i*17, 4);
						...
						if(shortAddress[0] == _currentShortAddress[0] && shortAddress[1] == _currentShortAddress[1]) {
							...
							if(!_protocolFailed) {
								...
								//**************************
								//send the range to Anchor
								//*************************
								for(uint16_t i=0; i< _networkDevicesNumber ; i++){
									byte* check = _networkDevices[i].getByteShortAddress();
									if(check[3] == 1){
										DW1000Device* anc;
										anc->setRXPower(DW1000.getReceivePower());
										anc->setRange(distance);

									    anc->setFPPower(DW1000.getFirstPathPower());
										anc->setQuality(DW1000.getReceiveQuality());
										Serial.println("Entered Transmition to Anchor");
										Serial.print("Anchor: "); 
										Serial.print(anc ->getShortAddress());
										transmitRangeReport(anc);
										Serial.println("Transmitted to Anchor");
									}
...
void DW1000RangingClass::timerTick() {
	if(_networkDevicesNumber > 0 && counterForBlink != 0) {
		...
		//Notify other anchors about them.
	if(_type == ANCHOR){
			transmitBlink();
		}
		//check for inactive devices if we are a TAG or ANCHOR
